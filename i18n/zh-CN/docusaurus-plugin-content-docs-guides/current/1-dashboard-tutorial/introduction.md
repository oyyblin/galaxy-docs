---
sidebar_label: 'Start with Dashboard'
sidebar_position: 1
slug: /
---

# Getting Started

## What is Galaxy Dashboard?

International Space Station is the backbone to the grand universe exploration, and so is our Dashboard to Galxe. Galaxy Dashboard is an easy-to-use yet powerful web editor for developers to make full use of our plug-and-play modules. You can curate credentials, power your own NFTs in metaverse, and launch campaigns by simply writing blogs.

## Kick start your first campaign on Galaxy:

1. [Set up a space on Galaxy](step-1-create-space.md)
2. [Curate the credential](step-2-curate-credentials.md)
3. [Deploy an NFT Core Contract](step-3-deploy-core.md)
4. [Upload an NFT template](step-4-upload-nft-template.md)
5. [Kickstart your campaign!](step-5-kickstart-campaign.md)

## A Hitchhiker guide to Galaxy Dashboard

[https://youtu.be/Dm-S-B8664U](https://youtu.be/Dm-S-B8664U)
